...
import { fetchRentalReport } from "../../../../lib/api";

/**
 * Serve o Relatório de Imóveis a Alugar (PDF) ao navegador.
 *
 * Route Handler, e não Server Action, porque o resultado é um ARQUIVO: o card
 * do hub vira um `<a target="_blank">` comum e o PDF abre no visualizador
 * nativo, sem trafegar binário em base64 nem esbarrar no bloqueador de pop-up.
 * Mesmo padrão do laudo de vistoria e do relatório de repasses.
 *
...
export async function GET(): Promise<Response> {
  ...
  const upstream = await fetchRentalReport().catch(() => null);
  ...
  if (!upstream || !upstream.ok) {
    ...
    const message =
      ...
      "Não foi possível gerar o relatório de imóveis a alugar.";
  ...
  return new Response(upstream.body, {
    headers: {
      ...
      "Content-Disposition": `inline; filename="imoveis-a-alugar.pdf"`,
